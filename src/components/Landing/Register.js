...
const Register = ({ location, history }, props) => {
			...
			switch (topage) {
				case 2:
					if (
						...
						notify.user("Register", "All fields are required", "info");
					}
					break;
				case 3:
					if (!validateEmail(email)) {
						notify.user("Register", "Input Valid Email", "info");
					} else if (
						email.trim() !== "" &&
						telephone.trim() !== "" &&
						dateOfBirth !== ""
	...
	const [school_town, setschool_town] = useState("");
			...
			AuthService.register(
				...
				(response) => {
					...
					AuthService.login(email, password).then(
						...
						() => {
							notify.user(
								"Registration",
								"Sorry! an error occured, try again ❗️",
								"danger"
							);
							setloading(false);
						}
				...
				(error) => {
					const resMessage =
						...
						error.message ||
						error.toString();
					notify.user("Register", resMessage.toString, "danger");
					setloading(false);
				}
			);
		}
								...
								>
									...
									{page === 3 ? (
										<span>
											...
											<div className="wrap-input100  m-b-16">
												<select
													className="custom-select input100"
													onChange={(e) => {
														setinitialRole(e.target.value);
													}}
												>
													<option disabled selected>
														Your Role e.g. Student
													</option>
													<option
														selected={initialRole === "student"}
														value="student"
													>
														Student
													</option>
													<option
														selected={initialRole === "educator"}
														value="educator"
													>
														Educator
													</option>
													<option
														selected={initialRole === "orgadmin"}
														value="orgadmin"
													>
														School Admin
													</option>
