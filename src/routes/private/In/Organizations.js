...
import SubEnvirons from "./SubEnvirons.js";
...
const orderorganizations = () => {
	UserService.getcreatedorganizations().then((response) => {
...
const Organizations = ({ user, history }) => {
	...
	const [noorganizations, setnoorganizations] = useState(tnoorganizations);
			...
			<Row>
				<Col md="12">
					<Card>
						<CardHeader>
							<Row>
								<Col md="10">Organizations</Col>
								<Col md="2">
									<ButtonGroup
										className="btn-group-toggle float-right"
										data-toggle="buttons"
									>
										<Button onClick={toggle} tag="label" color="info" size="sm">
								...
								<Col md="12">
										...
										<Table hover>
											...
											<tbody>
												{organizations.map((organization, key) => {
													return (
														<React.Fragment>
															<tr key={key} id={`org-${key}`}>
																<th scope="row">{key + 1}</th>
																<td><span className="theuser">{organization.name}</span></td>
																<td>{organization.address}</td>
																<td>{organization.code}</td>
																<td>{organization.environs.length}</td>
															</tr>
															<SubEnvirons
																environs={organization.environs}
																orgkey={`org-${key}`}
															/>
														</React.Fragment>
													);
									...
									{noorganizations ? (
										<div>
											...
											Add a New Organization to see it here.
										</div>
									) : (
										""
									)}
