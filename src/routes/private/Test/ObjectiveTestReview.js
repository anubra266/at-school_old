...
const ObjectiveTestReview = ({ user, match, history }) => {
  ...
  useEffect(() => {
    UserService.getobjectivetestreview(match.params.test).then((response) => {
      response.data.objectivequestions.forEach((question) => {
        question.objectiveoptions.sort((a, b) => 0.5 - Math.random());
      });
      settest(response.data);
    });
    UserService.getobjectivetestresult(match.params.test).then((response) => {
      const user_result = response.data.reduce((acc, nxt, index) => {
    ...
    <div className="content">
      ...
      <Row>
        <Col>
          {test && result && (
            <div>
              {test.objectivequestions.map((question, key) => {
                return (
                  <Card>
                    <CardBody>
                      <FormGroup>
                        ...
                        <div>
                          {question.objectiveoptions.map((option, key) => {
                            return (
                              <Row>
                                <Col>
                                  <CustomInput
              ...
              <Row>
                ...
                <Col md="2">
                  <ButtonGroup
                    ...
                    data-toggle="buttons"
                  >
                    <Button onClick={back} tag="label" color="info" size="sm">
                      Finish
                    </Button>
                  </ButtonGroup>
                </Col>
