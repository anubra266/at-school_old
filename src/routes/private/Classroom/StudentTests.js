...
const StudentTests = ({ history, educator, slug }) => {
  ...
  useEffect(() => {
    ...
    const updatetests = setInterval(() => {
      updateobjectivetests();
      updatetheorytests();
    }, 25000);
    updatetheorytests();
    return () => clearInterval(updatetests);
  }, []);
					...
					<Row>
						{theoryTests ? (
							theoryTests
								.filter(
									(theory) =>
										new Date(theory.deadline) > new Date() &&
										new Date(theory.starttime) <= new Date()
								)
								.slice(
						...
						{noobjective ? (
						  ...
						  objectiveTests.filter(
							...
							<Col sm="12">
								<div>
									<span className="text-info">
										There are upcoming Assessments{" "}
									</span>
									Be patient please.
								</div>{" "}
							</Col>
