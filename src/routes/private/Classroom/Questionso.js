...
const Questiona = ({ match, history }) => {
  ...
  const handleQuestionChange = (e, index) => {
    ...
    if (CurrentQuestion === test.objectivequestions.length - 1) {
      ...
      var thisoptions =
        test.objectivequestions[CurrentQuestion].objectiveoptions;
      if (
        thisquestion !== question ||
        thisoptions[0].option !== option1 ||
        thisoptions[1].option !== option2 ||
        thisoptions[2].option !== option3 ||
        thisoptions[3].option !== option4
      ) {
        settogo(index);
        togglewarning();
      } else {
  ...
  const updatetest = () => {
    UserService.getobjectivetest(match.params.test).then((response) => {
      response.data.objectivequestions.push({});
      settest(response.data);
      setCurrentQuestion(response.data.objectivequestions.length - 1);
      setQuestionsCount(
        Math.ceil(response.data.objectivequestions.length / pageSize)
      );
    });
  };
  useEffect(() => {
  ...
  const [solutioneditor, setsolutioneditor] = useState(null);
    ...
    if (question === "") {
      ...
      if (options.length < 4) {
        notify.user(
          ...
          "Two options cannot have same value!",
          "danger"
        );

        setloading(false);
      } else {
        UserService.addobjectivequestion(
          match.params.test,
          this_question,
          options
        ).then(
  ...
  const editquestion = () => {
    ...
    if (question === "") {
      ...
      if (options.length < 4) {
        ...
        UserService.updateobjectivequestion(
          test.objectivequestions[CurrentQuestion].id,
          this_question,
          options
        ).then(
          (response) => {
  ...
  const [uploadkey, setuploadkey] = useState(randomString);
    ...
    ExcelRenderer(fileObj, (err, resp) => {
      if (err) {
        ...
        if (newRows.length === 0) {
          ...
          console.log(newRows);
          UserService.addfromexcel(match.params.test, newRows).then(
            (response) => {
              notify.user("Add Questions to Test", response.data, "success");
              setuploadkey(randomString);
              setwait(false);
            ...
            (error) => {
              ...
              notify.user(
                "Add Questions to Test",
                resMessage + " - Make sure no cell is empty",
                "danger"
              );
              setuploadkey(randomString);
              setwait(false);
  ...
  const savesolution = (e) => {
    ...
    if (!test.objectivequestions[CurrentQuestion].objectivesolutions) {
      notify.user(
        "Set Test Solution",
        "You've not set a question here!",
        "danger"
      );
      setloadingsolution(false);
    } else if (
      !(test.objectivequestions[CurrentQuestion].objectivesolutions.length > 0)
    ) {
      UserService.addobjectivetestsolution(
        test.objectivequestions[CurrentQuestion].id,
        solution
      ).then(
        (response) => {
      ...
      UserService.updateobjectivetestsolution(
        test.objectivequestions[CurrentQuestion].objectivesolutions[0].id,
        solution
      ).then(
        (response) => {
          notify.user(
                      ...
                      <ModalBody>
                        <form onSubmit={savesolution}>
                          <FormGroup>
                            <CKEditor
                              ...
                              onInit={(solutiondata) => {
                                ...
                                solutiondata.setData(
                                  test.objectivequestions[CurrentQuestion]
                                    .objectivesolutions
                                    ? test.objectivequestions[CurrentQuestion]
                                        .objectivesolutions.length > 0
                                      ? test.objectivequestions[CurrentQuestion]
                                          .objectivesolutions[0].solution
                                      : initialsolution
                                    : initialsolution
                                );
                              }}
                          ...
                          <ModalFooter>
                            ...
                            <Button color="secondary" onClick={togglesolution}>
                              Cancel
                            </Button>
                          </ModalFooter>
                        </form>
                ...
                <Form method="POST" onSubmit={addquestion}>
                  <FormGroup tag="fieldset">
                    ...
                    <Label>
                      ...
                      Question, OptionA (Correct Option), OptionB, OptionC,
                      OptionD. E.g:
                  ...
                  {test.objectivequestions
                    .slice(
                      CurrentQuestion * pageSize,
                      (CurrentQuestion + 1) * pageSize
                    )
                    .map((objquestion, key) => {
                      return (
                        <span key={key}>
                          <FormGroup>
                            <p className="">
                              ...
                              <label>
                                - You can <strong>paste from word</strong>,
                                insert
                                <strong> pictures</strong> and
                            ...
                            <CKEditor
                              ...
                              onInit={(editor) => {
                                ...
                                editor.setData(
                                  objquestion.question || initialdata
                                );
                              }}
                          ...
                          <FormGroup tag="fieldset">
                            <p className="title">
                              Your Options{" "}
                              <label>
                                Insert correct answer in (A). Options would be
                                shuffled during test
                              </label>
                            </p>
                    ...
                    <div className="pagination-wrapper">
                      <Pagination aria-label="Page navigation example">
                        <PaginationItem disabled={CurrentQuestion <= 0}>
                          <PaginationLink
                            onClick={(e) =>
                              handleQuestionChange(e, CurrentQuestion - 1)
                            }
                            previous
                            href="#"
                          />
                        ...
                        {[...Array(QuestionsCount)].map((page, i) => (
                          <PaginationItem
                          ...
                          >
                            <PaginationLink
                              onClick={(e) => handleQuestionChange(e, i)}
                              href="#"
                            >
                              {i + 1}
                            </PaginationLink>
                  ...
                  <FormGroup>
                    {CurrentQuestion === test.objectivequestions.length - 1 ? (
                      <Button color="info" size="sm" disabled={loading}>
                        Add Question
                      </Button>
                    ) : (
                      <Button
                  ...
                  >
                    ...
                    <ModalBody>
                      <div className="aftertest">
                        <p className="info">
                          You've made changes. Save first, or you'll lose your
                          changes!
                        </p>
                      </div>
                    </ModalBody>
